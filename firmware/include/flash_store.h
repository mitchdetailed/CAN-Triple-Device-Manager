 ...
 *
 * Same user-visible consequence as ever: one re-Send after the update. */
/* v16: the four message passwords joined the header. Adding a field moves
 * HEADER_CRC_PREFIX and so changes the CRC of every image, which would reject a
 * v15 store on the checksum alone — true but accidental. The bump makes the
 * rejection say what it is. Table offsets are unchanged: they start at
 * HEADER_AREA, which has not moved. */
/* v17: AccessKeyRecord grew from 13 to 25 bytes for the four Protected Comms
 * slots, which moves every header field after it. */
#define FLASH_STORE_VERSION 17u
...
bool flash_store_commit(const uint16_t counts[FLASH_NUM_TABLES],
                        ...
                        const MessagePasswordRecord *msg_passwords);
...
bool flash_store_validate(uint16_t counts_out[FLASH_NUM_TABLES],
                          ...
                          MessagePasswordRecord *msg_passwords_out);
