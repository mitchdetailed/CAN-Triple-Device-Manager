...
/* v16: the four document-wide message passwords. The engine never CONSULTS
 * these — nothing on the device is gated on a message marking — it holds them
 * so that they are committed with the configuration and handed back on a Get,
 * which is what stops a round trip losing them. */
void engine_set_message_passwords(const MessagePasswordRecord *keys);
const MessagePasswordRecord *engine_message_passwords(void);
