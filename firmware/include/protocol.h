...
} CounterConfig;               /* 6*2 + 4*4 + 3 = 31 bytes */
...
/* v3: timer. Accumulates elapsed time (seconds) while running. Started/stopped
 * on the rising edge of a boolean channel. */
...
typedef struct {
    uint16_t start_signal_idx; /* rising edge starts the timer (0xFFFF = unused) */
    uint16_t stop_signal_idx;  /* rising edge stops the timer  (0xFFFF = unused) */
    ...
    uint8_t reserved;
} TimerConfig;                 /* 3*2 + 3*4 + 2 = 20 bytes */
