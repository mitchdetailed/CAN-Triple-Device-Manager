...
- **Source** names what *generates* the channel — what the value comes from, never where it is sent. A **receive** message decodes the frame and writes the channel, so it is named ("CAN 1 · Dash"). A **transmit** message is not: it *reads* the channel and puts it on the wire, so it is a destination, and this column does not report destinations. The one exception is a **Transmit CRC8**, which computes a checksum and publishes it into a channel ("CAN 1 · Dash CRC8" — see [Transmit CRC8](communications.md#crc8)); the device really does write that one. Otherwise: Math, User Condition, Counter, Timer, Integrator, Constant, Table 2x16, Table 8x8, or **Internal** for a **Device Channel**, which the unit produces itself whether or not anything uses it. Two entries separated by a comma mean two things write the channel — they share one slot and whichever runs last wins, which [Check Channels](validation-report.md) reports. **"not generated"** means nothing produces it yet: it may still be read (transmitted, or fed to a calculation), it just has no writer, so it carries its default value. **"unused"** is the stronger statement — nothing generates it *and* nothing reads it — and is exactly the set Check Channels offers to remove. A message or calculation that is switched off keeps a channel out of "unused" so cleanup cannot break it, but does not make it generated.
...
appears.</td></tr>
<tr><td><b>Range Minimum:</b> / <b>Range Maximum:</b></td><td>The channel's
declared range, and the <b>device's clamp</b> — every reading is limited to it.
It starts as what the chosen type spans at the chosen precision, and is
re-derived whenever the type or the decimals change; edit it to declare what
the channel should plausibly read — 0…8000 on an RPM channel — narrower or
wider than the type span alike. Minimum must stay below Maximum.</td></tr>
</table>
...
**Select…** opens **Select Input Channel** or **Select Output Channel**, depending on whether the field reads or writes. The picker searches by any part of the name or a regular expression, marks channels that have no generator yet or that are already written elsewhere, and its **Edit…** button — and, on an output, **New…** — opens Edit Custom Channel without leaving the picker. The same picker is used for math, condition, counter, timer, integrator and table inputs and outputs.

**Only an output can create a channel.** **Select Output Channel** has a **New…** button; **Select Input Channel** has none. What a field reads has to be produced somewhere first — a receive message row, a calculation, a constant — and a channel invented where it is only read would have nothing writing it: a transmit message row would send that channel's default value for ever, and a math input would read the same default. So the read side offers the catalogue and nothing beyond it. This covers the transmit comms row, math A/B/C, both sides of a condition, counter and timer triggers, an integrator's input, enable and reset, and a table axis.

**Nothing becomes harder to build — only differently ordered.** **Tools &gt; Channel Editor…** creates a channel from anywhere, and so does every output picker: a receive message row, a math or condition or counter or timer or integrator or table *output*. Define the channel at the thing that generates it, then read it wherever you like — which is the order the device runs in anyway. **Edit…** is on both sides throughout: correcting a channel's unit or range is a different matter from defining one that nothing writes.
