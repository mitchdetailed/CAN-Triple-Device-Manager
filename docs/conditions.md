...
> **Note:** Each active User Condition also takes one of the device's **1,000** channel slots for its output, and that — not room in the condition table — is the real reason the number is not higher: even 200 User Conditions can claim a fifth of the channel table on their own.

> **Note:** The limit was **250** in earlier versions. It came down to 200 to pay for the **Only when true for** duration below, which made each condition's record larger — and the trade came out ahead: the condition table is smaller than it was. If you have a configuration with more than 200 conditions, sending it reports the limit rather than dropping any silently.
...
## Only when true for — the durations

The **Set** box and the **Reset** box each have their own **Only when true for :**. Tick one, give it a time, and that side has to stay true for that long before it acts. "Coolant &gt; 105 for 5 seconds" is one condition; it does not need a timer standing in front of it.

The two are **independent**, which is the useful part. Put a duration on Set alone and the condition is slow to raise and quick to clear. Put one on Reset alone and it raises the instant the fault appears but only clears once the fault has been gone a while — the usual shape for something driving a warning lamp you do not want flickering.

Each side must hold *continuously*. If it goes false for a single evaluation that side's clock restarts from zero, so a value hovering on the threshold never accumulates its way through. Set and Reset keep separate clocks and never charge each other's.

Each channel-value comparison carries a **for the time set below** tick on its last line, and what they do depends on whether any is ticked in that box:
- **None ticked** — the duration applies to the **whole side**. With "Coolant &gt; 105 AND Load &gt; 80" for 5 seconds, both have to be true together for the whole five.
- **Some ticked** — it applies to **those comparisons only**, and the rest act immediately. Tick only the coolant one and it reads "coolant has been over 105 for five seconds, and load is over 80 right now". Ticked comparisons each keep their own clock, so they can come good at different moments.

**Message received and Message transmitted have no tick.** Those are events — true for the single evaluation pass in which the frame happened, false on the next — so "held continuously for five seconds" is something they can never be, and the tick disappears when you choose one. The duration for the whole side still applies and still means something: it is the finished result of the expression that has to hold, which a latch fed by an event can do.

> **Note:** **The longest duration is 655.35 seconds** — ten minutes and fifty-five — in steps of 0.01 s. That is the device's limit, not the dialog's: the duration travels to the unit as hundredths of a second in a two-byte field, which is what let both sides have one without costing any configuration memory. The step matches the device's calculation rate, so what you type is what it counts. For anything longer, use a [timer](timers.md) and compare it.

> **Note:** **Think before delaying a Reset.** Reset is dominant, and the point of that is a condition can always be cleared. A Reset duration holds the clear back, so a fault that has genuinely gone keeps the output raised for as long as you set. That is exactly what you want for de-bouncing a warning and exactly what you do not want for anything acting as an interlock. Both sides default to off.

> **Note:** Leave both boxes unticked and the condition behaves exactly as it always did. Configurations written before this existed load with both off.
...
**Deleting** the message is the one thing a rename cannot carry, and it is reported by **File → Check Channels** against the condition that names it — the same as a missing channel, and before a **Send** can fail on it. Timers and counter inputs that name messages are checked the same way.
