...
Online → Set Access Passwords… manages the passwords that live **in the device**, not in the file — which is why the dialog needs a connected CAN Triple. There are three functions, and **Protected Comms holds four slots**, so the list under **Function Passwords** shows six rows — Send a Configuration, Get a Configuration, and Protected Comms — Slot 1 to Slot 4 — each with ✓ = Password set. Select a row and click **Set…**.
...
<tr><td>Protected Comms<br>(Slots 1–4)</td><td><b>The send gate has nothing to confirm.</b> A configuration containing <a href="communications.md#marking">Protect Communication</a> messages goes only to a unit that confirms the matching Protected Comms password — and a unit with none set cannot confirm anything, so such a configuration is refused. Confirming <em>any</em> of the four slots satisfies the gate, which is what lets one unit accept sealed configurations from several suppliers without any of them sharing a password. The device also confirms this password before <i>this application</i> opens an existing Protect Communication message; the Message Password guarding that message is still required — this is not a master key over it. None of it stops the message being read off the device: a Get returns every record in full, to anyone, at every level.</td></tr>
</table>

The functions are independent: holding one proves nothing about the others. The Set… flow asks one question at a time — the old password when one is set ("Enter Old Password"), then the new password twice ("Enter New Password", "Confirm New Password"). Leaving the new password **blank clears it**, after a confirmation that spells out what anyone will then be able to do — clearing one Protected Comms slot of several says so too: the device goes on accepting the other slots' passwords. The device never reveals a password; it only reports which slots have one, and proving one is a challenge-response exchange, so nothing useful crosses the wire.
...
**So: set the password, then send a configuration.** That mattered little when nothing depended on these keys. It matters now, because Protected Comms is what a device confirms before a configuration containing Protect Communication messages can be sent to it, and before an existing Protect Communication message is opened here — a unit that lost it cannot answer for either. What a lost device password does *not* do is throw the markings open: each of those messages is still guarded by its **Message Password** — one of the configuration's four — and it is required whether the device answers or not.

> **Note:** Setting **Slot 1** also writes a matching verifier into the open document, so the file and the hardware agree about which password the send gate checks; Slots 2–4 are extra passwords the *device* accepts, and write nothing into the document. It is only that half — opening such a message here also takes the Message Password guarding it, and no Hidden or Read Only message answers to this password at any time. The document's protected messages must be revealed first (File → Reveal Protected Comms), and the configuration must be saved afterwards.
