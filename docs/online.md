...
Installs a `.ct3s` package on the connected device **without opening it**: the package's sealed install stream is relayed frame by frame and the device decrypts it, so nothing about the package reaches the document, the screen, or this process at all. The configuration you have open stays untouched. This is the command to use when the person at the laptop is entitled to install a configuration but not to read it.
...
- A package with **no install policy** is also refused, and so is a package with **no install stream** (one saved from the editor rather than built). "This package makes no demands" is not something a package is allowed to be; rebuild it with the Secure Configuration Builder.
- The policy is checked **before the device is touched** and before a single frame goes out — a package that does not belong on this unit must never get as far as partially overwriting it. A failure refuses with no override, and names the field and both values so you can tell whether you picked up the wrong file or the wrong unit.
- The Firmware Key is not a string comparison: the device must **prove** it by answering a challenge the package carries, so a look-alike reporting the right manufacturer and model still fails — and this Manager never holds the key. A unit with no licence cannot match anything, so it takes no packages at all — issue a licence first. See [Firmware Licensing &amp; Access Keys](licensing.md).
- No Send password is asked for. The sealed frames carry their own authority on the device, and if the package sets device passwords they are written from inside the stream, **before** the configuration, so they commit to flash with it.
- The install saves to flash and stamps the package version. There is no verify pass: the device answers every frame as it lands, and a refused frame ends the install with the frame named. A refusal also ends the sealed session on the device, so the install starts again from the beginning rather than resuming — "try again" is the whole remedy.
- Needs device firmware **1.0.8** or newer. An older unit refuses the first frame and the message says which firmware to update to. A package built by an older Manager (format 2) installs through the path it always used, with the Send password prompt and verify pass that path has.
