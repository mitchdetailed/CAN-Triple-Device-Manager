                                    ...
                                    bool resetAfter)
{
    ...
    // v16: the four document-wide message passwords. Sent unconditionally, for
    // the same reason device channels are: the payload is what CLEARS a slot a
    // previous configuration filled, so skipping the step when the document has
    // none would leave the old passwords on a device being reconfigured.
    {
        Step s;
        s.cmd = CMD_WRITE_MSG_PASSWORDS;
        s.payload = QByteArray(reinterpret_cast<const char *>(&tables.messagePasswords),
                               sizeof(tables.messagePasswords));
        s.stage = QStringLiteral("Sending message passwords");
        s.optional = true; // firmware without them NACKs — not fatal
        m_steps.append(s);
    }
    {
        Step s;
...
void ConfigTransfer::buildGetSteps()
{
    ...
    // with the firmware.
    // The four message passwords. Without them a Get returns marked messages
    // pointing at slots it cannot resolve — which is the round-trip loss this
    // whole table was added to end.
    {
        Step s;
        s.cmd = CMD_READ_MSG_PASSWORDS;
        s.stage = QStringLiteral("Reading message passwords");
        s.captureMsgPasswords = true;
        m_steps.append(s);
    }
    {
        Step s;
...
void ConfigTransfer::runNext()
{
    ...
    m_link->request(
        ...
        [this, self = QPointer<ConfigTransfer>(this)](
            ...
            if (step.captureMsgPasswords
                && payload.size() == int(sizeof(MessagePasswordRecord))) {
                std::memcpy(&m_readTables.messagePasswords, payload.constData(),
                            sizeof(MessagePasswordRecord));
            }
