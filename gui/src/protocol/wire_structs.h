...
struct TimerConfig {
    uint16_t start_signal_idx;  // 0xFFFF = unused
    uint16_t stop_signal_idx;
    uint16_t dest_signal_idx;
    float limit_value;
    float start_value;
    float stop_value;
...
static_assert(sizeof(CounterConfig) == 31, "must match firmware");
...
static_assert(sizeof(TimerConfig) == 20, "must match firmware");
