...
public:
    ...
    // requires the Protected Comms password to be proved against a CONNECTED
...
private:
    ...
    // Asked when a box is ticked on a section that has nothing guarding it yet.
    // False means the user backed out and the box must go back up.
    bool promptForFirstSectionPassword(CommsProtection level);
    ...
    // Deliberately does NOT accept the document's Protected Comms password
    ...
    // The Protected Comms password, checked BY A CONNECTED DEVICE. Demanded
    ...
    // A connected device has confirmed Protected Comms this editor. Set only
    ...
    // THE PASSWORD CHOSEN IN THIS VISIT, and the reason there is no longer a
    // Message Password field on the Parameters tab. A password is asked for at
    // the moment it is needed — taking a marking, releasing one — and nowhere
    // else, so there is nothing for a permanent field to show between times.
    // kNoAccessKey means "none chosen here"; syncParametersFromUi applies it.
    AccessKey m_pendingKey = kNoAccessKey;
