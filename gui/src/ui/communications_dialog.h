...
private:
    ...
    // The four document-wide message passwords. A marked message names one by
    // slot; see the tab for why they are document-wide rather than per bus.
    QWidget *buildPasswordsTab();
    void updatePasswordsTab();
    void onClearPasswordSlot(int slot);
    int messagesUsingPasswordSlot(int slot) const;
    void onSetPasswordSlot(int slot);
    QLabel *m_passwordState[Configuration::kCommsPasswordSlots] = {};
    // One action per slot: Set while empty, Clear while full.
    QPushButton *m_passwordAction[Configuration::kCommsPasswordSlots] = {};
    QLabel *m_passwordUsage = nullptr;
    ...
    // for Hidden, and for Protected that password AND an Protected Comms
    ...
    // Save the SELECTED sections as a .ct3t. Takes any message, concealed in
    // this session or not: the template carries the tier and the messageKey, so
    // a locked message stays locked on the other side and writing one out
    // discloses nothing. Editing is where protection is enforced — see
    // unlockConcealedSection() — not saving.
