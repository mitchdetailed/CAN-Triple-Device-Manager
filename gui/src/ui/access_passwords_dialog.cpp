...
void AccessPasswordsDialog::refreshList()
{
    ...
    for (int i = 0; i < kAccessFunctionCount; ++i) {
        ...
        const bool isSet = m_state.supported && m_state.isSet(fn);
        // The tick is part of the item's TEXT, so the colour applies to the
        // whole row rather than to the glyph alone — plain item text cannot be
        // partly coloured, and a custom delegate for one character would be a
        // lot of machinery for a three-row list. The unticked rows are padded
        // so both columns of labels still line up.
        auto *item = new QTreeWidgetItem(m_functionList);
        item->setText(0, (isSet ? QStringLiteral("✓ ") : QStringLiteral("   "))
...
bool AccessPasswordsDialog::runSetFlow(AccessFunction fn)
{
    const QString title = tr("Set Password");
    // Setting Edit Protected Comms writes a verifier into the open document as
    // well as a key into the device, so that the app and the unit agree about
    // which password opens a protected message.
    const bool touchesDocument = fn == AccessFunction::EditProtectedComms && m_config;
    ...
    if (touchesDocument && !m_config->commsRevealed()) {
        QMessageBox::warning(
            this, title,
            tr("This configuration's protected messages are still hidden, so its Edit Protected "
        ...
        if (first.isEmpty()) {
            ...
            if (QMessageBox::question(this, title,
                                      tr("Leave \"%1\" with no password?\n\n%2")
                                          .arg(accessFunctionLabel(fn), clearingMeans(fn)),
    ...
    bool written = false;
    {
        ...
        if (newPassword.isEmpty()) {
            written = device_session::clearAccessKey(m_link, fn, &error);
        } else {
            const AccessKey key = deriveAccessKey(newPassword);
            written = device_session::writeAccessKey(m_link, fn, key, &error);
        }
    }
    if (!written) {
        QMessageBox::warning(this, title,
    ...
    // the key.
    //
    // It costs everything now for Edit Protected Comms. It is the password a
    ...
    if (!committed) {
        // Said as the hazard it is, not as a footnote. The password IS set for
        // this session, so the dialog will show it ticked and the device will
        // check it — right up until the unit loses power.
        QMessageBox::warning(
            this, title,
            tr("This password is in force now, but it is not stored on the device yet.\n\n"
               "The device keeps its passwords in the same flash header as its "
               "configuration, and that header can only be written once each time the "
               "region is erased — which is what sending a configuration does. So on a "
               "unit that is already configured, a password lives in memory until the "
               "next Online → Send Configuration.\n\n"
               "Power the unit off before then and the password is gone, while this "
               "dialog goes on showing it as set. Send a configuration before "
               "disconnecting.\n\n"
               "This matters most for Edit Protected Comms: a device confirms it before "
               "this application will move a message's Protect Communication marking, so "
               "a unit that lost it can no longer supply that half of the check.\n\n"
               "(%1)")
                .arg(commitError));
    }

    // Step 5: the document, for Edit Protected Comms only. The other two are
    ...
    if (touchesDocument) {
        ...
        QMessageBox::information(
            this, title,
            newPassword.isEmpty()
                ? tr("The Edit Protected Comms password is cleared, on the device and in this "
                     "configuration.\n\nSave the configuration for this to take effect on the "
                     "file.")
                : tr("The Edit Protected Comms password is set, on the device and in this "
                     "configuration.\n\nSave the configuration for this to take effect on the "
                     "file."));
    }
    return true;
