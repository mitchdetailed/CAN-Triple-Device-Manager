...
void MainWindow::updateProtectionState()
{
    ...
    if (m_revealAction) {
        ...
        m_revealAction->setToolTip(
            ...
            hasPassword ? tr("Enter the Edit Protected Comms password to prove it for this "
                             ...
                             "full either way.")
                        : tr("This configuration has no Edit Protected Comms password, so "
...
bool MainWindow::ensureDeviceAccess(AccessFunction fn)
{
    ...
    // Edit Protected Comms is the one the operator may legitimately never have
...
bool MainWindow::openPath(const QString &path)
{
    ...
    // key is wrapped under the Edit Protected Comms password, and a pre-v8 plain
    ...
    if (peek.requiresPassword) {
        ...
        // one is worse than naming none: someone hunting for an "Edit Protected
...
void MainWindow::onRevealProtectedComms()
{
    ...
    for (;;) {
        ...
        const QString password = QInputDialog::getText(
            ...
            tr("Enter the Edit Protected Comms password to see and change the messages "
...
bool MainWindow::onSaveSecureConfig()
{
    ...
    // A document with no Edit Protected Comms password can set one right here.
    //
    ...
    auto describe = [](bool requirePassword) -> QString {
        if (requirePassword)
            return tr("The file key is wrapped under the Edit Protected Comms password, so the "
    ...
    const auto refresh = [&] {
        ...
        if (needNewPassword && passwordEdit->text().isEmpty())
            warning = tr("Choose the Edit Protected Comms password this file will require.");
    ...
    if (options.requirePassword && hasCommsPassword) {
        ...
        for (;;) {
            ...
            const QString password = QInputDialog::getText(
                ...
                tr("Enter the Edit Protected Comms password this file will require:"),
                QLineEdit::Password, QString(), &ok);
            if (!ok)
                return false;
    ...
    if (passwordApplied) {
        if (!m_config.setCommsPassword(passwordEdit->text())) {
            QMessageBox::warning(this, tr("Save Secure Config"),
                                 tr("The Edit Protected Comms password could not be applied."));
            return false;
        }
...
// CONNECTED DEVICE confirming the Edit Protected Comms password, and this is the
...
bool MainWindow::proveProtectedCommsForEdit()
{
    if (!m_link.isOpen()) {
        QMessageBox::warning(
            ...
            tr("Unticking Protect Communication needs the Edit Protected Comms password "
    ...
    if (!state.supported || !state.isSet(AccessFunction::EditProtectedComms)) {
        return QMessageBox::question(
                   this, tr("Protect Communication"),
                   tr("This device has no Edit Protected Comms password set, so there is "
...
void MainWindow::onCommunications()
{
    ...
    // requires the Edit Protected Comms password to be proved against a
    ...
    if (peek.requiresPassword) {
        for (;;) {
            ...
            password = QInputDialog::getText(
                this, title,
                tr("\"%1\" requires its Edit Protected Comms password before it can be "
                   "installed.\n\nPassword:")
                    .arg(QFileInfo(path).fileName()),
                QLineEdit::Password, QString(), &ok);
            if (!ok)
...
void MainWindow::onSendConfiguration()
{
    ...
    // A Protected section is unticked by proving Edit Protected Comms AGAINST A
        ...
        if (readAccess && access.supported
            ...
            if (QMessageBox::warning(
                    ...
                    tr("This configuration marks messages Protect Communication, but this "
                       "device has no Edit Protected Comms password set.\n\n"
                       "Unticking that marking is authorised by a device confirming that "
...
// said Edit Protected Comms unlocks a refused table read, on the strength of a
...
// a 2.3.0 unit, so it is offered first and named in the prompt. Edit Protected
...
bool MainWindow::offerProtectedCommsUnlock(const QString &title)
{
    ...
    // Edit Protected Comms password set returns true here without asking
...
void MainWindow::onSetAccessPasswords()
{
    if (!ensureConnected())
        return;
    // The document goes in as well as the link: setting Edit Protected Comms
...
void MainWindow::onAbout()
{
    QMessageBox::about(this, tr("About CAN Triple Device Manager"),
                       tr("<h3>CAN Triple Device Manager %1</h3>"
                          ...
                          "(STM32G473, 3× CAN). Layout and workflow inspired by "
                          "MoTeC Dash Manager.</p>"
