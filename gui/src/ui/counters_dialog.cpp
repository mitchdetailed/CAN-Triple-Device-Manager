...
public:
        ...
        : QDialog(parent), m_config(config), m_row(row), m_livePatch(livePatch)
    {
        ...
        buildSourceRow(inputForm, QObject::tr("Up :"), m_up);
        buildSourceRow(inputForm, QObject::tr("Down :"), m_down);
        ...
        m_rateRow = rateRow;
        buildSourceRow(inputForm, QObject::tr("Reset :"), m_reset);
        buildSourceRow(inputForm, QObject::tr("Enable :"), m_enable);
        ...
        // what the row produces reads as the conclusion of what it is built
        // from, not as a question asked before any of it is on screen. It is
        // only the PLACEMENT that moved - the controls are still built and
        // wired above, so nothing about when the field is populated changed.
        ...
        loadSource(m_up, m_row.upSource, m_row.upChannel);
        loadSource(m_down, m_row.downSource, m_row.downChannel);
        ct::setChannelField(m_followEdit, m_row.followChannel, m_config->catalog());
        loadSource(m_reset, m_row.resetSource, m_row.resetChannel);
        loadSource(m_enable, m_row.enableSource, m_row.enableChannel);
        ...
        rememberWindowSize(this, QStringLiteral("counterRow")); // sized to content by default; a size the user chose wins
    }

    CounterRow result() const { return m_row; }

private:
    // One input that can be a channel OR a message event.
    //
    // The kind combo sits FIRST, before the operand, because it decides what the
    // operand even is — a reader going left to right meets "what kind" before
    // "which one", and the channel picker and the message list occupy the same
    // place so exactly one is ever visible.
    struct SourceRow
    {
        QComboBox *kind = nullptr;
        QLineEdit *edit = nullptr;     // channel operand
        QPushButton *select = nullptr;
        QComboBox *message = nullptr;  // message operand
    };

    static QString msgKey(int bus, const QString &name)
    {
        return QStringLiteral("%1\u0001%2").arg(bus).arg(name);
    }

    void buildSourceRow(QFormLayout *form, const QString &label, SourceRow &row)
    {
        QWidget *parent = form->parentWidget();
        auto *rowLayout = new QHBoxLayout;

        row.kind = new QComboBox(parent);
        row.kind->addItem(QObject::tr("Channel"), int(ct::COUNTER_SRC_SIGNAL));
        row.kind->addItem(QObject::tr("Message received"), int(ct::COUNTER_SRC_MSG_RX));
        row.kind->addItem(QObject::tr("Message transmitted"), int(ct::COUNTER_SRC_MSG_TX));
        rowLayout->addWidget(row.kind);

        row.edit = new QLineEdit(parent);
        row.edit->setReadOnly(true);
        row.select = new QPushButton(QObject::tr("Select\u2026"), parent);
        rowLayout->addWidget(row.edit, 1);
        rowLayout->addWidget(row.select);

        row.message = new QComboBox(parent);
        for (int b = 0; b < 3; ++b)
            for (const CommsSection &s : m_config->bus[b].sections)
                if (!s.name.isEmpty())
                    row.message->addItem(QStringLiteral("CAN%1 \u00b7 %2").arg(b + 1).arg(s.name),
                                         msgKey(b + 1, s.name));
        rowLayout->addWidget(row.message, 1);

        form->addRow(label, rowLayout);

        SourceRow *rp = &row;
        QObject::connect(row.kind, &QComboBox::currentIndexChanged, this,
                         [this, rp](int) { syncSource(*rp); });
        QObject::connect(row.select, &QPushButton::clicked, this, [this, rp] {
            const QString picked = SelectChannelDialog::pickInput(
                m_config, ct::channelField(rp->edit), this, m_livePatch);
            if (!picked.isEmpty())
                ct::setChannelField(rp->edit, picked, m_config->catalog());
        });
        syncSource(row);
    }

    void setSourceEnabled(SourceRow &row, bool on)
    {
        row.kind->setEnabled(on);
        row.edit->setEnabled(on);
        row.select->setEnabled(on);
        row.message->setEnabled(on);
    }

    void syncSource(SourceRow &row)
    {
        const bool message = row.kind->currentData().toInt() != int(ct::COUNTER_SRC_SIGNAL);
        row.edit->setVisible(!message);
        row.select->setVisible(!message);
        row.message->setVisible(message);
    }

    void loadSource(SourceRow &row, const ct::CounterSource &src, const QString &channel)
    {
        const int at = row.kind->findData(src.kind);
        row.kind->setCurrentIndex(at >= 0 ? at : 0);
        if (src.isMessage()) {
            const int m = row.message->findData(msgKey(src.messageBus, src.message));
            if (m >= 0)
                row.message->setCurrentIndex(m);
        } else {
            ct::setChannelField(row.edit, channel, m_config->catalog());
        }
        syncSource(row);
    }

    void readSource(const SourceRow &row, ct::CounterSource *src, QString *channel) const
    {
        *src = ct::CounterSource{};
        *channel = QString();
        const int kind = row.kind->currentData().toInt();
        if (kind == int(ct::COUNTER_SRC_SIGNAL)) {
            *channel = ct::channelField(row.edit);
            return;
        }
        src->kind = kind;
        const QStringList parts =
            row.message->currentData().toString().split(QChar(char16_t(0x0001)));
        if (parts.size() == 2) {
            src->messageBus = parts.at(0).toInt();
            src->message = parts.at(1);
        }
    }
    ...
    void updateTypeEnables()
    {
        ...
        // The KIND combo follows the mode too, not just the operand: a row that
        // cannot be used should not let its source be changed either.
        setSourceEnabled(m_up, !follow && !rate);
        setSourceEnabled(m_down, !follow && !rate);
    ...
    void onOk()
    {
        ...
        // ALL FIVE ROWS ARE READ BACK, INCLUDING THE ONES THIS TYPE GREYS OUT,
        // and that is deliberate. The unused inputs are kept so that switching
        // Up/Down to Follow and back does not destroy the channel that was
        // typed — the rule ConditionRow already follows for its unused
        // expression half.
        //
        // What was actually wrong was downstream: the mapper, the validator and
        // both cross-reference walks each decided for themselves which inputs a
        // counter reads, and all of them let Follow and Up/Down see each other's
        // leftovers. They now ask CounterRow::readsUpDown()/readsFollow(), so a
        // kept input stays dormant instead of blocking a Send or claiming a
        // channel it never reads.
        readSource(m_up, &m_row.upSource, &m_row.upChannel);
        readSource(m_down, &m_row.downSource, &m_row.downChannel);
        m_row.followChannel = ct::channelField(m_followEdit);
        readSource(m_reset, &m_row.resetSource, &m_row.resetChannel);
        readSource(m_enable, &m_row.enableSource, &m_row.enableChannel);
    ...
    // Follow keeps a plain channel row: it reads a VALUE, and a frame arriving
    // has none, so there is nothing for a message kind to mean here.
...
{
    QStringList parts;
    // A MESSAGE INPUT IS AN INPUT, and this column used to render channels
    // only — so a counter driven entirely by frames showed an empty Inputs
    // cell, reading as a counter with nothing driving it. The arrow marks it as
    // an event rather than a value, and the direction says which way the frame
    // was going.
    const auto describe = [&](const ct::CounterSource &src, const QString &channel) {
        if (src.isMessage()) {
            const QString arrow = src.kind == int(ct::COUNTER_SRC_MSG_TX)
                                      ? QStringLiteral("tx")
                                      : QStringLiteral("rx");
            return src.message.isEmpty()
                       ? QStringLiteral("(no message)")
                       : QStringLiteral("%1←%2").arg(arrow, src.message);
        }
        return channel.isEmpty() ? QString() : catalog.labelFor(channel);
    };
    const auto put = [&](const QString &label, const QString &text) {
        if (!text.isEmpty())
    ...
    } else if (row.readsFollow()) {
        if (!row.followChannel.isEmpty())
            parts << QStringLiteral("follow=%1").arg(catalog.labelFor(row.followChannel));
    } else {
        put(QStringLiteral("up"), describe(row.upSource, row.upChannel));
        put(QStringLiteral("down"), describe(row.downSource, row.downChannel));
    }
    put(QStringLiteral("reset"), describe(row.resetSource, row.resetChannel));
    put(QStringLiteral("en"), describe(row.enableSource, row.enableChannel));
    ...
    : QDialog(parent), m_config(config), m_rows(config->counterRows)
{
    ...
    setModal(true);
    resize(700, 400);
    rememberWindowSize(this, QStringLiteral("counters")); // the default, until the user changes it
...
void CountersDialog::onMove(int delta)
{
