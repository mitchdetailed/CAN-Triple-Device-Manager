...
//
// Protected Comms is the one with a foot in both worlds: it gates the
...
public:
    ...
    // clearing Protected Comms updates the document to match.
...
private:
    ...
    bool runSetFlow(AccessFunction fn, int slot);
    AccessFunction selectedFunction() const;
    // v17: which Protected Comms slot the selected row means (1..4); 1 for the
    // single-slot functions.
