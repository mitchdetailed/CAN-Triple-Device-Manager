...
// `maxChunks` is the script table of the firmware the document targets — its
// capacity's Script entry — so a variant with a larger table takes a larger
// script, and one with none refuses every script by name.
    ...
    if (chunkCount > maxChunks) {
        if (error) {
            *error = QObject::tr("The device script is %1 bytes, more than the "
                                 "%2 bytes this device can store.")
                         .arg(image.size())
                         .arg(maxChunks * int(sizeof(ScriptChunk)));
        }
        return false;
    }
...
{
    chunks->clear();
    const int maxChunks = config.capacity().capacityOf(DeviceTable::Script);
    ...
    if (!config.hasScriptSource()) {
        ...
        if (!validateScriptImage(retained, &reason,
                                 config.capacity().capacityOf(DeviceTable::Signals))) {
            if (error) {
                *error = QObject::tr(
        ...
        return chunkImage(retained, chunks, error, maxChunks);
    }
    ...
    return chunkImage(r.image, chunks, error, maxChunks);
}
...
private:
    ...
    QByteArray buildImage()
    {
        ...
        // Run the DEVICE's verifier on our own output, against the channel
        // table of the firmware the symbols were resolved for. A compiler that
        // emitted an image the device would refuse is a compiler bug; catching
        // it here means it never reaches a vehicle, and the failure names the
        // exact check rather than arriving as "the script did not load".
        const uint8_t verdict = script_verify(image.constData(), uint32_t(image.size()),
                                              uint16_t(m_symbols.signalSlots));
