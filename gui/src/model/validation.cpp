...
QList<ValidationIssue> validateConfiguration(const Configuration &config)
{
    ...
    for (int busIdx = 0; busIdx < 3; ++busIdx) {
        ...
        const auto checkSection = [&](const CommsSection &s) {
            ...
            // v16: A MARKING MUST NAME ONE OF THE FOUR MESSAGE PASSWORDS.
            //
            // The wire carries a slot NUMBER, so a key that is not one of the
            // four has nothing to travel as: mapToDevice writes slot 0, which is
            // the honest value and also means the message arrives on the unit
            // with no password at all. Caught here so it is a Check Channels
            // finding about the row rather than something discovered after a
            // Send and a Get, when the marking has already come home unguarded.
            //
            // The section editor cannot create this state — it only accepts a
            // password that matches a slot — but a document can arrive in it from
            // a hand-edited file or a template.
            if (s.protection != CommsProtection::None && s.device != SectionDevice::Off) {
                if (s.messageKey == kNoAccessKey) {
                    add(ValidationIssue::Error, loc,
                        QStringLiteral("marked %1 but has no Message Password")
                            .arg(commsProtectionToken(s.protection)));
                } else if (config.commsPasswordSlotFor(s.messageKey) == 0) {
                    add(ValidationIssue::Error, loc,
                        QStringLiteral("marked %1 with a Message Password that is not one of "
                                       "the four set in Communications Setup — it cannot be "
                                       "sent to a device")
                            .arg(commsProtectionToken(s.protection)));
                }
            }
    ...
    // A counter's message input, checked the way a User Condition's message
    // operand is: not a channel, so it is looked up among the document's
    // messages instead. Same two failures, same severity — nothing selected is
    // an unfinished input, and a name no section matches is a reference to
    // something that has gone. Both used to surface for the first time as
    // "cannot map the configuration to the device", which refuses the whole
    // Send and names the mapper rather than the row.
    const auto checkMessageRef = [&](int bus, const QString &name, const QString &loc,
                                     const QString &side) {
        if (name.isEmpty()) {
            add(ValidationIssue::Error, loc,
                QStringLiteral("%1: no message selected").arg(side));
            return;
        }
        bool found = false;
        const int b = bus - 1;
        if (b >= 0 && b <= 2)
            for (const CommsSection &ms : config.bus[b].sections)
                if (ms.device != SectionDevice::Off && !ms.isRelay()
                    && ms.name.compare(name, Qt::CaseInsensitive) == 0) {
                    found = true;
                    break;
                }
        if (!found)
            add(ValidationIssue::Error, loc,
                QStringLiteral("%1: CAN %2 has no message named '%3'")
                    .arg(side)
                    .arg(bus)
                    .arg(name));
    };
    const auto checkCounterMessage = [&](const ct::CounterSource &src, const QString &loc,
                                         const QString &side) {
        checkMessageRef(src.messageBus, src.message, loc, side);
    };
    for (int i = 0; i < config.counterRows.size(); ++i) {
        ...
        if (c.outputChannel.isEmpty())
            add(ValidationIssue::Error, loc, QStringLiteral("no output channel"));
        // Only what the MODE reads, asked through the row so this walk cannot
        // drift from the mapper's. Walking an input the counter does not read
        // would report it as referencing a channel it demonstrably does not,
        // which is how an unused channel stays alive in Check Channels forever.
        const bool isRate = (c.mode == ct::COUNTER_MODE_RATE);
        if (c.readsUpDown()) {
            // A MESSAGE INPUT IS NOT A CHANNEL, so each side is checked as one
            // or the other, never both. Nothing checked the message sources at
            // all before, which left two states passing Check Channels and then
            // failing the Send: a message kind with nothing selected (reachable
            // directly — the picker is empty on a document with no sections),
            // and one naming a section since deleted or renamed.
            if (c.upSource.isMessage())
                checkCounterMessage(c.upSource, loc, QStringLiteral("up"));
            else
                checkInput(c.upChannel, loc, QStringLiteral("up"));
            if (c.downSource.isMessage())
                checkCounterMessage(c.downSource, loc, QStringLiteral("down"));
            else
                checkInput(c.downChannel, loc, QStringLiteral("down"));
        }
        if (c.readsFollow())
            checkInput(c.followChannel, loc, QStringLiteral("follow"));
        if (c.resetSource.isMessage())
            checkCounterMessage(c.resetSource, loc, QStringLiteral("reset"));
        else
            checkInput(c.resetChannel, loc, QStringLiteral("reset"));
        if (c.enableSource.isMessage())
            checkCounterMessage(c.enableSource, loc, QStringLiteral("enable"));
        else
            checkInput(c.enableChannel, loc, QStringLiteral("enable"));
        // "Neither an Up nor a Down" has to count MESSAGE inputs too. A counter
        // driven entirely by frames leaves both channel fields empty, so this
        // warned about a counter that was configured perfectly well.
        const bool hasUp = c.upSource.isMessage() || !c.upChannel.isEmpty();
        const bool hasDown = c.downSource.isMessage() || !c.downChannel.isEmpty();
        if (c.readsUpDown() && !hasUp && !hasDown)
            add(ValidationIssue::Warning, loc,
                QStringLiteral("up/down counter has neither an Up nor a Down input"));
        if (c.readsFollow() && c.followChannel.isEmpty())
            add(ValidationIssue::Warning, loc,
    ...
    for (int i = 0; i < config.timerRows.size(); ++i) {
        ...
        if (t.outputChannel.isEmpty())
            add(ValidationIssue::Error, loc, QStringLiteral("no output channel"));
        // Both operands of both triggers. A message operand is not a channel, so
        // it is looked up among the document's messages instead — the same
        // check a User Condition's operand and a counter's input get. This used
        // to defer to the mapper, which meant a trigger naming a message that
        // had been deleted or renamed said nothing at Check Channels and then
        // refused the whole Send. Renaming now repoints these (see
        // Configuration::renameMessageReferences), so what is left to catch is
        // a deletion — and it is caught at the row that has the problem.
        const auto checkTerm = [&](const ConditionTermRow &tr, const QString &side) {
            if (tr.isMessageOp()) {
                checkMessageRef(tr.aMessageBus, tr.aMessage, loc, side);
                return;
            }
            checkInput(tr.aChannel, loc, side);
            if (tr.bIsChannel)
                checkInput(tr.bChannel, loc, side + QStringLiteral(" comparison"));
        };
        checkTerm(t.startTerm, QStringLiteral("start"));
        checkTerm(t.stopTerm, QStringLiteral("stop"));

        // "Set but empty" is what makes a trigger never fire, and it is asked of
        // whichever operand the operator actually reads.
        const auto unset = [](const ConditionTermRow &tr) {
            return tr.isMessageOp() ? tr.aMessage.isEmpty() : tr.aChannel.isEmpty();
        };
        if (unset(t.startTerm) && unset(t.stopTerm))
            add(ValidationIssue::Warning, loc,
                QStringLiteral("timer has neither a start nor a stop trigger — it never runs"));
        else if (unset(t.startTerm))
            add(ValidationIssue::Warning, loc,
                QStringLiteral("timer has no start trigger — it never starts"));
