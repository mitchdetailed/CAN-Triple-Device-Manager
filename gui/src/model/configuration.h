...
public:
    ...
    // Protected Comms. Its reach is now NARROWER than a reader would guess
    ...
    // The DOCUMENT-WIDE Protected Comms state, and nothing more. True when
    ...
    // is true for every document that has no Protected Comms password —
    ...
    struct SectionProofs {
        ...
        bool deviceProof;     // Protected Comms, confirmed by a CONNECTED device
    };
    // None       nothing.
    ...
    //            answer to Protected Comms alone, which made it the one tier
    ...
    // is stranded, and both writers save such a document — as they now save any
    // document, concealed sections and all; see anyKeyedSectionConcealed().
    //
    ...
    // carries no Protected Comms verifier, so on precisely the documents
    ...
    // Set, change or clear the document's Protected Comms password; an
    ...
    // Does anything that would actually REACH the device carry the Protected
    // marking? Sections switched Off are excluded because mapToDevice skips
    // them outright — a marking on a message that is not being sent is not a
    // reason to refuse the send.
    //
    // Used by the Send gate: a configuration carrying Protected messages goes
    // only to a unit that proves the same Protected Comms key.
    bool hasProtectedComms() const;
    void setCommsKey(AccessKey key) { m_commsKey = key; }

    // ---------------------------------------------- the four message passwords
    //
    // DOCUMENT-WIDE, and there are exactly four. Every marked message points at
    // one of them rather than carrying a password of its own, which is what
    // makes a marking cheap enough to survive a device round trip: the wire
    // stores a three-bit SLOT INDEX in a byte CanMessageConfig already had, and
    // the four keys themselves ride in the config header. A per-message key
    // needed its own table and 800 bytes of a region with 1,504 to spare.
    //
    // Held as derived keys, never as passwords, exactly like every other secret
    // in this class. Slot numbering is 1..kCommsPasswordSlots for the user and
    // for the wire; 0 means "no password", which is what an unmarked message
    // has. The array is 0-based, so slotKey() does the arithmetic once.
    static constexpr int kCommsPasswordSlots = 4;
    AccessKey commsPassword(int slot) const
    {
        return (slot >= 1 && slot <= kCommsPasswordSlots) ? m_commsPasswords[slot - 1]
                                                          : kNoAccessKey;
    }
    void setCommsPasswordSlot(int slot, AccessKey key)
    {
        if (slot >= 1 && slot <= kCommsPasswordSlots) {
            m_commsPasswords[slot - 1] = key;
            setDirty();
        }
    }
    // Which slot holds this key, or 0. The lookup a marked section needs in both
    // directions: to check that a typed password is one of the four, and to say
    // which slot the wire should carry.
    int commsPasswordSlotFor(AccessKey key) const
    {
        if (key == kNoAccessKey)
            return 0;
        for (int i = 0; i < kCommsPasswordSlots; ++i)
            if (m_commsPasswords[i] == key)
                return i + 1;
        return 0;
    }
    // The first slot with nothing in it, or 0 when all four are taken. What a
    // template load asks before it starts placing marked messages.
    int firstFreeCommsPasswordSlot() const
    {
        for (int i = 0; i < kCommsPasswordSlots; ++i)
            if (m_commsPasswords[i] == kNoAccessKey)
                return i + 1;
        return 0;
    }
    int commsPasswordsInUse() const
    {
        int n = 0;
        for (int i = 0; i < kCommsPasswordSlots; ++i)
            if (m_commsPasswords[i] != kNoAccessKey)
    ...
    // config_file.h. Saves ANY document, including one holding messages this
    // session cannot read: the body is sealed, so nothing legible leaves, and
    // the tier and the messageKey travel with each section so what reopens is
    // the same padlocked row. Both writers refused this until format 2 made
    // "written out in the clear" impossible — see the note at the refusal's
    // former site for what that argument was and why it expired. The marking is
    // enforced at the editor, not at the writer.
    bool saveToFile(const QString &path, QString *error = nullptr);
    // Binary .ct3s — see secure_file.h. Same rule as saveToFile, and this
    // writer never had the clear-text problem at all: a .ct3s was always sealed,
    // so its refusal rested purely on principle and the principle is now applied
    // where it bites.
    ...
    // The same job for a MESSAGE name. A condition, a timer trigger and a
    // counter input can all key off "this message was received", and those
    // references are held by name and bus rather than by any stable id — so
    // renaming a message in Communications Setup used to orphan every one of
    // them silently, exactly as renaming a channel would have without the call
    // above.
    //
    // `bus` is 1-BASED, matching what the rows store and what the mapper keys
    // on (device_mapper's messageRefKey(busIdx + 1, ...)), not the 0-based
    // index into bus[].
    //
    // Called by applyBusSections, which is the only writer of the document's
    // sections and already works out which incoming section is which prior one.
    // There is no messageRenamed signal to match channelRenamed: a channel can
    // be renamed from a picker nested inside an open grid, whereas a message
    // name only changes when Communications Setup commits, and every grid is
    // opened with exec(). If one of them ever becomes modeless, this needs the
    // signal too.
    int renameMessageReferences(int bus, const QString &oldName, const QString &newName);
    ...
    //
    // A KEYED concealed section was the laundering case the refusal was written
    // for: the session cannot read it, a real password for it is in existence,
    // and re-emitting its CAN ID and bit layout IN THE CLEAR would have handed
    // the protocol to somebody who could not open it. Format 2 removed the "in
    // the clear", and with it the refusal — both writers now save such a
    // document, and what they write stays shut.
    //
    // The predicate is kept because the question is still a real one and the
    // tests ask it: "is this document holding something this session cannot
    // read, that somebody's password could open". It gates nothing today. Any
    // future caller that needs to reason about that state should use this
    // rather than re-deriving it, and a caller that wants to REFUSE on it
    // should read the note at the former refusal site first.
    ...
    //                       device confirming Protected Comms. The device
