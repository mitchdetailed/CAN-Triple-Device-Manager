...
ReportBuilder buildReport(const Configuration &config)
{
    ...
    // Hidden, that plus a device confirming Edit Protected Comms for Protected.
    ...
    // with no Edit Protected Comms password, so it ORed away the per-section
    ...
    for (int b = 0; b < 3; ++b) {
        ...
        for (int si = 0; si < bus.sections.size(); ++si) {
            ...
            // confirming Edit Protected Comms — and a reader deciding whether it
        ...
        for (int i = 0; i < config.counterRows.size(); ++i) {
            ...
            r.subhead(QStringLiteral("    (%1) %2 - %3 counter (min %4, max %5, step %6%7)%8")
                          ...
                          .arg(c.outputChannel, kind,
                               ...
                               inactiveTag(c.active)));
            // A rate counter reads none of the three counting inputs, so they
            // are left out rather than listed as channels it uses.
            usesGenerates({{isRate ? QString() : c.upChannel, QStringLiteral("up input")},
                           {isRate ? QString() : c.downChannel, QStringLiteral("down input")},
                           {isRate ? QString() : c.followChannel, QStringLiteral("follow input")},
    ...
    if (!config.timerRows.isEmpty()) {
        ...
        for (int i = 0; i < config.timerRows.size(); ++i) {
            ...
            usesGenerates({{t.startChannel, QStringLiteral("start input")},
                           {t.stopChannel, QStringLiteral("stop input")}},
...
ChannelUsage analyzeChannelUsage(const Configuration &config)
{
    ...
    for (int i = 0; i < config.counterRows.size(); ++i) {
        ...
        // Rate mode is clock-driven and reads none of the counting inputs; the
        // cross-reference must agree with the mapper, or Check Channels reports
        // a channel as still in use by a counter that never looks at it.
        const bool cntRate = (cnt.mode == ct::COUNTER_MODE_RATE);
        const QList<QPair<QString, QString>> inputs = {
            {cntRate ? QString() : cnt.upChannel, QStringLiteral(" up")},
            {cntRate ? QString() : cnt.downChannel, QStringLiteral(" down")},
            {cntRate ? QString() : cnt.followChannel, QStringLiteral(" follow")},
    ...
    for (int i = 0; i < config.timerRows.size(); ++i) {
        ...
        if (!t.active) {
            c.dormantRef(t.startChannel);
            c.dormantRef(t.stopChannel);
            c.dormantRef(t.outputChannel);
            continue;
        }
        if (!t.startChannel.isEmpty())
            c.user(t.startChannel, where + QStringLiteral(" start"));
        if (!t.stopChannel.isEmpty())
            c.user(t.stopChannel, where + QStringLiteral(" stop"));
