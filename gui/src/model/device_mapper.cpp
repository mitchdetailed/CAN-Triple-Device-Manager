...
MappingResult mapToDevice(const Configuration &config)
{
    ...
    for (int busIdx = 0; busIdx < 3; ++busIdx) {
        ...
        for (const CommsSection &section : busCfg.sections) {
            ...
            if (section.isTransmit()) {
                ...
                // v16: WHICH OF THE FOUR DOCUMENT PASSWORDS GUARDS THIS MESSAGE, as a
                // slot number. 0 for an unmarked message, and 0 too for a marked one
                // whose key is not one of the four - which validation refuses before a
                // Send gets here, so reaching it means the document is already wrong
    ...
    for (int i = 0; i < config.conditionRows.size(); ++i) {
        ...
        // Milliseconds to CENTISECONDS, rounded to nearest and clamped to what
        // a u16 can say (655.35 s). The editor's 0.01 s step means the rounding
        // never actually fires; it is here because a document can arrive from a
        // hand-edited file or an older writer.
        const auto toCs = [](int ms) {
            return quint16(qBound(0, (ms + 5) / 10, 65535));
        };
        cc.set_qualify_cs = toCs(qMax(0, c.qualifySetMs));
        cc.reset_qualify_cs = toCs(qMax(0, c.qualifyResetMs));
        // Masked to the terms that expression actually HAS. A bit standing for a
        // comparison that was deleted would qualify a term the device never
        // evaluates, and that side would wait forever for it.
        const auto termBits = [](int n) {
            return (1 << qMin(n, int(COND_MAX_TERMS))) - 1;
        };
        // ...and masked again to the comparisons that CAN honour a hold time.
        // A message operator is an EVENT: true for the single evaluation pass
        // in which the frame arrived and false on the next. A bit standing for
        // one asks the device to hold an instant for two seconds, which it can
        // never do, so that side waits forever and nothing anywhere says why.
        //
        // THE CONDITIONS EDITOR ALREADY REFUSES TO SET SUCH A BIT, and this is
        // deliberately not a duplicate of that rule. The editor only runs when
        // somebody opens a row and presses OK; a document reaches the device by
        // many other roads — a file written before that rule existed, a secure
        // package installed without ever opening the row, a readback from a
        // device configured by an older build, a firmware-update restore. All
        // of them arrive at this line. mapToDevice() is the chokepoint every
        // write-to-device path crosses, so it is where the rule actually holds;
        // the editor's copy is what keeps the on-screen tick honest while you
        // are looking at it.
        const auto timeableBits = [](const QList<ConditionTermRow> &terms) {
            int bits = 0;
            for (int t = 0; t < terms.size() && t < int(COND_MAX_TERMS); ++t)
                if (!terms[t].isMessageOp())
                    bits |= (1 << t);
            return bits;
        };
        cc.set_qualify_terms = quint8(c.qualifySetTerms & termBits(int(c.setTerms.size()))
                                      & timeableBits(c.setTerms));
        cc.reset_qualify_terms = quint8(c.qualifyResetTerms & termBits(int(c.resetTerms.size()))
                                        & timeableBits(c.resetTerms));
    ...
    // v16: THE FOUR DOCUMENT-WIDE MESSAGE PASSWORDS. Copied verbatim, empty
    // slots included — an empty slot is what CLEARS one a previous configuration
    // left on the unit, and a marked message's slot number only means anything
    // against the same four it was chosen from.
    for (int s = 1; s <= Configuration::kCommsPasswordSlots; ++s)
        r.tables.messagePasswords.key[s - 1] = quint32(config.commsPassword(s));
    ...
    for (int i = 0; i < config.counterRows.size(); ++i) {
        ...
        CounterConfig cfg{};
        // ONLY THE INPUTS THIS MODE READS are resolved. Leaving the rest at
        // SIG_MSG_NONE keeps a stale reference from a type change out of the
        // record, and out of the reference walks that would otherwise report
        // the counter as reading a channel it no longer looks at.
        //
        // This used to exclude Rate alone, which was right about Rate and wrong
        // about the other two: a Follow counter still resolved its leftover Up
        // and Down, and an Up/Down counter still resolved its leftover Follow.
        // Resolving is not free — a message that has since been deleted is an
        ...
        const bool isRate = (c.mode == COUNTER_MODE_RATE); // rate_hz / direction below

        // A MESSAGE input puts a message index where a signal index would go and
        // records its kind in input_kinds; a channel input is untouched, which is
        // why COUNTER_SRC_SIGNAL is 0 and an old record needs no migration.
        //
        // Refused rather than skipped when the message has gone, for the reason
        // a condition refuses it: an input that silently read false would leave
        // a counter that never counts, and the configuration would look as
        // though it had mapped cleanly.
        cfg.input_kinds = 0;
        const auto resolveInput = [&](const CounterSource &src, const QString &channel,
                                      const QString &side, int shift) -> quint16 {
            if (!src.isMessage())
                return resolveBoolInput(channel, where, side);
            if (src.message.isEmpty()) {
                r.errors.append(QStringLiteral("%1 %2: no message selected").arg(where, side));
                return SIG_MSG_NONE;
            }
            const auto mit =
                messageIndexByBusName.constFind(messageRefKey(src.messageBus, src.message));
            if (mit == messageIndexByBusName.constEnd()) {
                r.errors.append(QStringLiteral("%1 %2: CAN %3 has no message named %4")
                                    .arg(where, side)
                                    .arg(src.messageBus)
                                    .arg(src.message));
                return SIG_MSG_NONE;
            }
            cfg.input_kinds |= quint8(quint8(src.kind) << shift);
            return quint16(*mit);
        };

        cfg.up_signal_idx = c.readsUpDown() ? resolveInput(c.upSource, c.upChannel,
                                                           QStringLiteral("up"),
                                                           kCounterSrcShiftUp)
                                            : SIG_MSG_NONE;
        cfg.down_signal_idx = c.readsUpDown() ? resolveInput(c.downSource, c.downChannel,
                                                             QStringLiteral("down"),
                                                             kCounterSrcShiftDown)
                                              : SIG_MSG_NONE;
        // Follow keeps reading a channel: it tracks a VALUE, and a frame
        // arriving has none.
        cfg.follow_signal_idx =
            c.readsFollow() ? resolveBoolInput(c.followChannel, where, QStringLiteral("follow"))
                            : SIG_MSG_NONE;
        cfg.reset_signal_idx = resolveInput(c.resetSource, c.resetChannel,
                                            QStringLiteral("reset"), kCounterSrcShiftReset);
        cfg.enable_signal_idx = resolveInput(c.enableSource, c.enableChannel,
                                             QStringLiteral("enable"), kCounterSrcShiftEnable);
    ...
    for (int i = 0; i < config.timerRows.size(); ++i) {
        ...
        // ONE TERM PER SIDE, and an EMPTY one is legal here where it is not in
        // a condition: a timer with no Stop simply never stops itself, which is
        // what an empty Stop channel always meant. Unused is spelled by an
        // out-of-range operand, the same convention the old 0xFFFF used, and
        // the device reads it as false forever.
        const auto mapTimerTerm = [&](const ConditionTermRow &tr, const QString &side,
                                      ConditionTerm *out) -> bool {
            *out = ConditionTerm{};
            out->input_a_signal_idx = kTimerTermUnused;
            const bool blank = tr.isMessageOp() ? tr.aMessage.isEmpty() : tr.aChannel.isEmpty();
            if (blank)
                return true; // unused half
            out->op = quint8(tr.op);
            if (tr.isMessageOp()) {
                const auto mit =
                    messageIndexByBusName.constFind(messageRefKey(tr.aMessageBus, tr.aMessage));
                if (mit == messageIndexByBusName.constEnd()) {
                    // Refused, not skipped, for the reason a condition refuses
                    // it: a term that silently read false would leave a timer
                    // that never starts, and the configuration would look as
                    // though it had mapped cleanly.
                    r.errors.append(QStringLiteral("%1 %2: CAN %3 has no message named %4")
                                        .arg(where, side)
                                        .arg(tr.aMessageBus)
                ...
                out->input_a_signal_idx = quint16(*mit);
                return true;
            }
            ...
            if (tr.bIsChannel) {
                if (tr.bChannel.isEmpty()) {
                    r.errors.append(QStringLiteral("%1 %2: no channel to compare against")
                                        .arg(where, side));
                    return false;
                }
                ...
                out->input_b_type = 1;
                out->b.input_b_idx = quint16(idxB);
            } else {
                out->input_b_type = 0;
                out->b.input_b_const = float(tr.bConst);
            }
            return true;
        };

        TimerConfig cfg{};
        bool termsOk = mapTimerTerm(t.startTerm, QStringLiteral("start"), &cfg.start_term);
        termsOk = mapTimerTerm(t.stopTerm, QStringLiteral("stop"), &cfg.stop_term) && termsOk;
        if (!termsOk)
            continue;
                   ...
                   const QVector<ControlCanPayload> &busSetup)
{
    ...
    // also kept the session's Protected Comms grant, so the sections the
    ...
    for (int ci = 0; ci < tables.conditions.size(); ++ci) {
        ...
        c.qualifySetMs = int(cc.set_qualify_cs) * 10;
        c.qualifyResetMs = int(cc.reset_qualify_cs) * 10;
        c.qualifySetTerms = int(cc.set_qualify_terms);
        c.qualifyResetTerms = int(cc.reset_qualify_terms);
    ...
    for (const CounterConfig &cfg : tables.counters) {
        ...
        c.mode = isRate ? COUNTER_MODE_RATE
                        : (cfg.mode == COUNTER_MODE_FOLLOW) ? COUNTER_MODE_FOLLOW
                                                            : COUNTER_MODE_UPDOWN;
        // The mirror of resolveInput(). A MESSAGE input's index names a message,
        // so turning it back into a channel name would invent one from a number
        // that means something else entirely.
        const auto readInput = [&](quint16 idx, int shift, CounterSource *src) -> QString {
            const int kind = (cfg.input_kinds >> shift) & 0x3;
            *src = CounterSource{};
            if (kind == COUNTER_SRC_SIGNAL)
                return boolName(idx);
            const int mi = int(idx);
            if (msgSection.contains(mi) && mi < tables.messages.size()) {
                const int busIdx = int(tables.messages[mi].src_bus) - 1;
                if (busIdx >= 0 && busIdx <= 2) {
                    src->kind = kind;
                    src->messageBus = busIdx + 1;
                    src->message = config.bus[busIdx].sections[msgSection.value(mi)].name;
                }
            }
            return QString();
        };
        c.upChannel = readInput(cfg.up_signal_idx, kCounterSrcShiftUp, &c.upSource);
        c.downChannel = readInput(cfg.down_signal_idx, kCounterSrcShiftDown, &c.downSource);
        c.followChannel = boolName(cfg.follow_signal_idx);
        c.resetChannel = readInput(cfg.reset_signal_idx, kCounterSrcShiftReset, &c.resetSource);
        c.enableChannel =
            readInput(cfg.enable_signal_idx, kCounterSrcShiftEnable, &c.enableSource);
    ...
    for (const TimerConfig &cfg : tables.timers) {
        if (!(cfg.flags & TIMERFLAG_ACTIVE))
            continue;
        // The mirror of mapTimerTerm(). An out-of-range operand is the unused
        // half and comes back as a blank term rather than as a channel name
        // invented from an index that names nothing.
        const auto readTimerTerm = [&](const ConditionTerm &wt) {
            ConditionTermRow tr;
            if (condOpIsMessage(wt.op)) {
                const int mi = int(wt.input_a_signal_idx);
                if (msgSection.contains(mi) && mi < tables.messages.size()) {
                    ...
                    if (busIdx >= 0 && busIdx <= 2) {
                        tr.op = wt.op;
                        tr.aMessageBus = busIdx + 1;
                        tr.aMessage = config.bus[busIdx].sections[msgSection.value(mi)].name;
                    }
                }
                return tr;
            }
        ...
        t.startTerm = readTimerTerm(cfg.start_term);
        t.stopTerm = readTimerTerm(cfg.stop_term);
    ...
    // v16: THE FOUR PASSWORDS COME HOME, and every marked message's key is
    // rebuilt from the slot it points at.
    //
    // The document wins where it already has an answer: the reconciliation pass
    // above has restored messageKey for every section this document recognised,
    // and its copy may be NEWER than the unit's — somebody changed a password
    // and has not sent yet. This fills only what that pass could not, which is
    // the case the whole feature exists for: a Get into a fresh document, where
    // before the marking came back and the password did not.
    for (int s = 1; s <= Configuration::kCommsPasswordSlots; ++s) {
        const AccessKey fromDevice = AccessKey(tables.messagePasswords.key[s - 1]);
        if (fromDevice != kNoAccessKey && config.commsPassword(s) == kNoAccessKey)
            config.setCommsPasswordSlot(s, fromDevice);
    }
    for (int m = 0; m < tables.messages.size(); ++m) {
        const int slot = int(tables.messages[m].password_slot);
        if (slot < 1 || slot > Configuration::kCommsPasswordSlots)
            continue;
        const int bus = int(tables.messages[m].src_bus) - 1;
        const auto at = msgSection.constFind(m);
        if (bus < 0 || bus > 2 || at == msgSection.constEnd())
            continue;
        if (*at < 0 || *at >= config.bus[bus].sections.size())
            continue;
        CommsSection &sec = config.bus[bus].sections[*at];
        if (sec.messageKey == kNoAccessKey)
            sec.messageKey = config.commsPassword(slot);
    }
