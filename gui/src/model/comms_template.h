...
//
// THAT IS WHY SAVING DOES NOT ASK PERMISSION. A concealed message can be
// written into a template without being unlocked first, which looks alarming
// until the marking is followed through: the message arrives locked, needing
// the same password it needed before, and the person who wrote it out never
// saw its contents either. A template moves a locked box; it does not open one.
// Opening is unlockConcealedSection()'s job and it still demands the Message
// Password, plus the Protected Comms password proved against a connected
// device when the tier is Protect Communication.
//
// ---------------------------------------------------------------------------
...
namespace ct {
class Configuration;
// Kept equal to Configuration::kCommsPasswordSlots by a static_assert in the
// .cpp. Declared here rather than including configuration.h, which would pull
// the whole document model into a header the document model itself uses.
constexpr int kTemplatePasswordSlots = 4;
} // namespace ct
...
struct CommsTemplate
{
    ...
    // v16: THE MESSAGE PASSWORDS THE MARKED SECTIONS IN THIS FILE NAME.
    //
    // A marked message names one of its document's four passwords by SLOT, and a
    // slot number means nothing on its own - it is an index into a table that
    // belongs to the configuration it came from. So the template carries the
    // KEYS, in the slots they occupied at the source, and loading re-homes them
    // into whatever slots the destination has free.
    //
    // Only the slots the file's own marked sections actually use are filled; the
    // rest stay kNoAccessKey. A template of unmarked messages carries none, and
    // loads into any configuration.
    AccessKey passwords[kTemplatePasswordSlots] = {kNoAccessKey, kNoAccessKey,
...
// v16: CAN THIS DOCUMENT HOME THE PASSWORDS THIS TEMPLATE NEEDS?
//
// Asked BEFORE mergeCommsTemplate, and that order is the whole point: merging
// CREATES CHANNELS in the catalog as it goes, so a refusal discovered afterwards
// would already have changed the document. "Nothing is applied" has to mean
// nothing, and the only way to promise that is to ask first.
//
// A slot already holding the identical password is a MATCH, not a conflict, so
// loading the same template twice - or two templates from one source
// configuration - costs one slot rather than two. A password the destination
// does not have needs a free slot; with none, this reports which password could
// not be placed and the caller applies nothing.
bool commsTemplatePasswordsFit(const Configuration &config, const CommsTemplate &tmpl,
                               QString *error = nullptr);

// Move the template's passwords into this document's free slots. Call only
