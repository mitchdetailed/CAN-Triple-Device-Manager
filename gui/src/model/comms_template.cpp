...
const QLatin1String kKeyPasswords("messagePasswords");
...
bool writeCommsTemplate(const QString &path, const CommsTemplate &tmpl, QString *error)
{
    ...
    // The message passwords the marked sections here name, in the slots they had
    // at the source. Always four entries, empties included, so a slot number
    // always means the same slot.
    {
        QJsonArray pw;
        for (int i = 0; i < kTemplatePasswordSlots; ++i)
            pw.append(double(tmpl.passwords[i]));
        body[kKeyPasswords] = pw;
    }
...
static_assert(kTemplatePasswordSlots == Configuration::kCommsPasswordSlots,
              "a template's password table must be the document's");

// Which of the template's password slots its own marked sections actually use.
// A template can carry a slot nothing names - the source document had four and
// this file uses one - and homing an unused password would spend a slot in the
// destination for nothing.
static QList<AccessKey> templatePasswordsUsed(const CommsTemplate &tmpl)
{
    QList<AccessKey> used;
    for (const CommsSection &s : tmpl.sections) {
        if (s.protection == CommsProtection::None || s.messageKey == kNoAccessKey)
            continue;
        if (!used.contains(s.messageKey))
            used.append(s.messageKey);
    }
    return used;
}
...
bool commsTemplatePasswordsFit(const Configuration &config, const CommsTemplate &tmpl,
                               QString *error)
{
    const QList<AccessKey> used = templatePasswordsUsed(tmpl);
    if (used.isEmpty())
        return true; // nothing marked: loads into any configuration

    // Counted rather than placed, because placing would mean mutating the
    // document to find out whether the load is possible. An identical password
    // already in a slot needs no room - that is what makes loading the same
    // template twice, or two templates from one source, cost one slot.
    int need = 0;
    for (const AccessKey k : used)
        if (config.commsPasswordSlotFor(k) == 0)
            ++need;
    int free = 0;
    for (int slot = 1; slot <= Configuration::kCommsPasswordSlots; ++slot)
        if (config.commsPassword(slot) == kNoAccessKey)
            ++free;
    if (need <= free)
        return true;
    ...
    if (error) {
        *error = QObject::tr(
                     "This template's messages are guarded by %1 Message Password(s) that "
                     "this configuration does not have, and only %2 of its four slots are "
                     "free.\n\nNothing has been loaded. Free a slot on the Passwords tab "
                     "of Communications Setup, or remove the markings from the template.")
                     .arg(need)
                     .arg(free);
    }
    return false;
...
{
    for (const AccessKey k : templatePasswordsUsed(tmpl)) {
        if (config.commsPasswordSlotFor(k) != 0)
            continue; // already here, in whatever slot this document uses
        const int slot = config.firstFreeCommsPasswordSlot();
        if (slot == 0)
            return; // commsTemplatePasswordsFit() should have refused this load
        config.setCommsPasswordSlot(slot, k);
    }
}
