...
struct CommsSection {
    ...
    //              rather than reveals.
    //   Protected  As Hidden, and ALSO needs the Protected Comms password
...
struct ConditionRow {
    ...
    int latchHz = 10;
    // THE "FOR" QUALIFIER, ONE PER EXPRESSION, in milliseconds. 0 is off, and a
    // condition with both at 0 behaves exactly as every condition did before it
    // existed.
    //
    // That side must be continuously true for this long before it counts as
    // true. RESET DEFAULTS TO 0 — immediate — because Reset is dominant and a
    // latch that cannot be cleared promptly is the wrong way for one to fail;
    // the delay is there for "clear only once the fault has been gone a while",
    // which somebody has to ask for rather than get by accident.
    //
    // Held in MILLISECONDS here and carried as centiseconds on the wire. The
    // editor's step is 0.01 s, so the two cannot disagree by rounding, and the
    // device's ceiling of 655.35 s is what the spin box enforces.
    int qualifySetMs = 0;
    int qualifyResetMs = 0;
    // WHICH comparisons each applies to — bit i for that expression's term i. 0
    // means the whole folded expression ("X > 5 AND Y > 2, for five seconds");
    // non-zero qualifies just those terms and leaves the rest instant ("X > 5
    // for five seconds AND Y > 2 right now"). The two differ under AND and both
    // get asked for, so they are masks rather than flags.
    int qualifySetTerms = 0;
    int qualifyResetTerms = 0;
...
// Where one counter input comes from. A channel as before, or a message event.
//
// ADDITIVE, and deliberately: the channel fields on CounterRow stay exactly what
// they were, so every rename walk, report and validation path that already knows
// how to follow a counter's channel keeps working untouched. Only the places
// that need to understand a message consult this. Folding the channel into this
// struct would have been tidier and would have touched a hundred call sites to
// say the same thing they already say.
//
// `kind` is ct::CounterSrc. When it is not Signal the channel field is ignored
// and (bus, message) names the frame — by NAME, never an index, for the reason
// ConditionTermRow gives.
struct CounterSource
{
    int kind = 0;
    int messageBus = 0; // 1..3; 0 = unset
    QString message;

    bool isMessage() const { return kind != 0; }

    QJsonObject toJson() const;
    static CounterSource fromJson(const QJsonObject &o);
};
...
struct CounterRow {
    ...
    // v15: the four inputs that can take a message event instead of a channel.
    // Follow is absent because it reads a VALUE and a frame arriving has none.
    CounterSource upSource;
    CounterSource downSource;
    CounterSource resetSource;
    CounterSource enableSource;
    double minValue = 0;
    ...
    // WHICH INPUTS THE MODE ACTUALLY READS — asked here so that every consumer
    // asks the same question and they cannot drift apart.
    //
    // The unused inputs are KEPT in the row rather than cleared when the type
    // changes, which is the rule ConditionRow already follows for its unused
    // expression half: switching Up/Down to Follow and back must not destroy
    // the channel that was typed. The cost of keeping is that "does this
    // counter read this input" stops being obvious from the field alone, and
    // four separate places used to guess. They all guessed the same wrong way
    // — they excluded Rate (which reads none of the three) and let Follow and
    // Up/Down see each other's inputs. A Follow counter whose leftover Up named
    // a since-deleted message therefore failed the whole Send, and Check
    // Channels listed a channel as in use by a counter that never looks at it.
    // Defined in configuration.cpp, beside ConditionTermRow::isMessageOp() and
    // for the same reason: this header does not pull in the wire structs, and
    // the mode constants belong to them.
    bool readsUpDown() const;
    bool readsFollow() const;
...
struct TimerRow {
    QString outputChannel;
    // START AND STOP ARE COMPARISONS, not bare channels. Each is the same term
    // a User Condition is built from, so a timer keys off "RPM > 4000" or "this
    // message was received" without a condition row standing in between — and
    // the device evaluates both through one function, so the two cannot drift
    // apart about what a comparison means.
    //
    // The EDGE is unchanged: the term yields a boolean and the timer triggers on
    // its rising edge. A pre-schema-20 timer is therefore exactly the term
    // (channel NEQ 0), which is what fromJson() migrates it into.
    //
    // A term with no channel and no message is UNUSED — that half never fires,
    // which is what an empty Start or Stop always meant.
    ConditionTermRow startTerm;
    ConditionTermRow stopTerm;
    ...
    // Every channel either term reads, for the rename and validation walks.
    // Message operands are not channels; see messageRefs().
    QStringList inputChannels() const;
    // Every message either term names, as (bus, section name) pairs.
    QList<QPair<int, QString>> messageRefs() const;

    QJsonObject toJson() const;
    // Accepts the schema-20 term form and the older "start"/"stop" channel
    // names, which become (channel NEQ 0) — the comparison the device was
    // already making.
