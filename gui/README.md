...
- **Online → Set Access Passwords…** — function passwords held
  **in the device**: Send a Configuration, Get a Configuration, and **Protected
  ...
  nothing next time. Send and Get are real device gates; Protected Comms is
  what a unit confirms before a configuration containing **Protect
  Communication** messages will be sent to it, and before this application
  opens an existing Protected message. The Message Password guarding that
  message is still required; this is not a master key over it.
- **Message markings** (section editor: **Read Only**, **Hidden**, **Protect
  ...
  visible so a customer can use the values. **A marked message is guarded by one
  of the configuration's four Message Passwords**, chosen on the Passwords tab
  of Communications Setup; moving a marking costs the password guarding it —
  raising as well as lowering — and a message wears one marking at a time. Opening a hidden message does not leave it open:
