...
- Table capacities, mirrored in `wire_structs.h` from the firmware's own
  header and asserted equal against it in the full tree: messages **500** (see the 9-bit
...
Commands (`wire_structs.h` carries the host's copy): GET_STATUS 0x01, WRITE/READ
...
hand-bit-packing the eight small fields behind accessors on both sides of the
wire, and narrowing `mux_id`/`mux_mask` to 16 bits. The
...
first. `MAX_MESSAGES` in `wire_structs.h` says so at the definition. A transmit message carries
...
CANTripleDeviceManager/
  ...
  firmware/include/           the interface the app links against, and only
                              that: script_vm.h, script_exec.h, fw_image.h and a
                              protocol.h reduced to one struct, generated by
                              stripping the firmware's own headers. The firmware
                              is closed source; its wire, store and licence
                              design are not published. wire_structs.h is the
                              host's own copy of everything that crosses the
                              wire, and is the authority here.
```
