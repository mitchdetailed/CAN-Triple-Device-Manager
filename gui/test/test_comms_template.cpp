...
// A marked message all the way into a document, through the commit path the
// dialog actually uses. testAMarkedMessageStaysMarked stops at the merge, which
// is where the tier is preserved; this goes one step further to
// applyBusSections(), which is the chokepoint that can REFUSE.
void testAMarkedMessageLoadsIntoADocument(const QTemporaryDir &dir)
{
    for (const auto tier : {CommsProtection::Hidden, CommsProtection::Protected}) {
        const char *label = tier == CommsProtection::Hidden ? "Hidden" : "Protected";

        Configuration source;
        buildSource(source);
        source.bus[0].sections[0].protection = tier;
        source.bus[0].sections[0].messageKey = deriveAccessKey(QStringLiteral("supplier"));

        CommsTemplate tmpl;
        REQUIRE(roundTrip(source, templatePath(dir, tier == CommsProtection::Hidden ? "mload-h" : "mload-p"), &tmpl));
        REQUIRE(!tmpl.sections.isEmpty());
        CHECK(tmpl.sections.first().protection == tier);
        ...
        QString refusal;
        const bool applied = target.applyBusSections(1, merge.sections, &refusal);
        if (!applied)
            std::printf("   [%s] applyBusSections REFUSED: %s\n", label, qPrintable(refusal));
        CHECK(applied);
        CHECK(target.bus[1].sections.size() == 1);
        if (!target.bus[1].sections.isEmpty()) {
            const CommsSection &landed = target.bus[1].sections.first();
            CHECK(landed.protection == tier);
            CHECK(landed.messageKey == deriveAccessKey(QStringLiteral("supplier")));
            CHECK(landed.isConcealed(target.isSectionRevealed(landed, 1)));
        }
    }
}

// The everyday shape: the document ALREADY holds a marked message that this
// session has not unlocked, and a template is loaded alongside it.
void testALoadIntoADocumentHoldingConcealedMessages(const QTemporaryDir &dir)
{
    Configuration source;
    buildSource(source);
    source.bus[0].sections[0].protection = CommsProtection::Hidden;
    source.bus[0].sections[0].messageKey = deriveAccessKey(QStringLiteral("supplier"));
    CommsTemplate tmpl;
    REQUIRE(roundTrip(source, templatePath(dir, "into-locked"), &tmpl));

    // A target already carrying its own concealed, marked message on the SAME
    // bus the template is going onto, with no grant taken for it.
    Configuration target;
    buildSource(target);
    target.bus[1].enabled = true;
    target.bus[1].sections = target.bus[0].sections;
    target.bus[1].sections[0].name = QStringLiteral("Existing Locked");
    target.bus[1].sections[0].protection = CommsProtection::Hidden;
    target.bus[1].sections[0].messageKey = deriveAccessKey(QStringLiteral("other"));
    REQUIRE(target.bus[1].sections.first().isConcealed(
        target.isSectionRevealed(target.bus[1].sections.first(), 1)));

    CommsTemplateMerge merge;
    REQUIRE(mergeCommsTemplate(target.catalog(), 1, tmpl, &merge, nullptr));

    // What the dialog does: append to the working copy, then commit the whole bus.
    QList<CommsSection> next = target.bus[1].sections;
    next.append(merge.sections);

    QString refusal;
    const bool applied = target.applyBusSections(1, next, &refusal);
    if (!applied)
        std::printf("   [into-locked] REFUSED: %s\n", qPrintable(refusal));
    CHECK(applied);
    CHECK(target.bus[1].sections.size() == next.size());
}
...
// A CONCEALED MESSAGE IS WRITTEN, NOT REFUSED — the inverse of what this used
// to assert, and the whole of the change. Saving moves a locked box; the
// password is what opens it, and opening is somebody else's button.
void testTheSaveButtonWritesAConcealedMessage(const QTemporaryDir &dir)
{
    Configuration config;
    buildSource(config);
    ...
    // Never unlocked in this session, which is the point: no grant is taken and
    // no password is typed anywhere in this test.
    REQUIRE(config.bus[0].sections[0].isConcealed(
        config.isSectionRevealed(config.bus[0].sections[0], 0)));
    ...
    DialogPilot pilot;
    const QString path = dir.filePath(QStringLiteral("locked.ct3t"));
    pilot.filePath = path;
    save->click();
    ...
    // The confirmation says the file carries a locked message rather than
    // leaving the person handing it on to find out from the recipient.
    CHECK(pilot.allText().contains(QStringLiteral("keep their marking")));

    // AND IT ARRIVES LOCKED. A file that quietly unlocked what it carried would
    // pass every check above and be the actual disaster, so the marking and the
    // key are read back out rather than assumed.
    CommsTemplate back;
    QString error;
    REQUIRE(readCommsTemplate(path, &back, &error));
    REQUIRE(!back.sections.isEmpty());
    CHECK(back.sections.first().protection == CommsProtection::Hidden);
    CHECK(back.sections.first().messageKey == deriveAccessKey(QStringLiteral("supplier")));

    Configuration fresh;
    fresh.clear();
    CommsTemplateMerge merge;
    REQUIRE(mergeCommsTemplate(fresh.catalog(), 0, back, &merge, nullptr));
    REQUIRE(!merge.sections.isEmpty());
    CHECK(merge.sections.first().isConcealed(/*revealed=*/false));
    QString refusal;
    REQUIRE(fresh.applyBusSections(0, merge.sections, &refusal));
    CHECK(fresh.bus[0].sections.first().isConcealed(
        fresh.isSectionRevealed(fresh.bus[0].sections.first(), 0)));
    // The password that opens it on the other side is the one that opened it
    // here — not a new one, and not none.
    CHECK(fresh.bus[0].sections.first().messageKey
...
// PASSWORD SLOTTING IS DYNAMIC, and a load never overwrites a slot.
//
// The slot a password occupied at the SOURCE means nothing at the destination:
// the file carries the keys, and loading re-homes each one the file's marked
// sections actually use into whatever slot the destination has free. Two
// examples, both stated by the product owner:
//
//   1. every destination slot taken, template needs passwords it does not
//      have -> the load is REFUSED and nothing is applied
//   2. destination slots 1 and 2 taken, template uses two other passwords
//      (in ITS slots 1 and 2) -> they land in 3 and 4, and 1 and 2 are
//      untouched
void testTemplatePasswordSlottingIsDynamic(const QTemporaryDir &dir)
{
    const AccessKey X = deriveAccessKey(QStringLiteral("template-pw-x"));
    const AccessKey Y = deriveAccessKey(QStringLiteral("template-pw-y"));
    const AccessKey A = deriveAccessKey(QStringLiteral("dest-pw-a"));
    const AccessKey B = deriveAccessKey(QStringLiteral("dest-pw-b"));

    // The source document: X and Y in its slots 1 and 2, guarding two marked
    // messages. Saved and re-read, so the keys travel the way a real .ct3t
    // carries them.
    Configuration source;
    source.bus[0].enabled = true;
    source.setCommsPasswordSlot(1, X);
    source.setCommsPasswordSlot(2, Y);
    CommsSection hid;
    hid.name = QStringLiteral("Hidden One");
    hid.device = SectionDevice::ReceiveMessage;
    hid.baseAddress = 0x640;
    hid.messageLengthBytes = 8;
    hid.protection = CommsProtection::Hidden;
    hid.messageKey = X;
    CommsSection ro = hid;
    ro.name = QStringLiteral("ReadOnly One");
    ro.baseAddress = 0x641;
    ro.protection = CommsProtection::ReadOnly;
    ro.messageKey = Y;
    source.bus[0].sections.append(hid);
    source.bus[0].sections.append(ro);

    CommsTemplate tmpl = buildCommsTemplate(source.catalog(), source.bus[0],
                                            source.bus[0].sections,
                                            QStringLiteral("slotting"));
    for (int s = 1; s <= Configuration::kCommsPasswordSlots; ++s)
        tmpl.passwords[s - 1] = source.commsPassword(s); // as the save path does

    const QString file = dir.filePath(QStringLiteral("slotting.ct3t"));
    QString error;
    REQUIRE(writeCommsTemplate(file, tmpl, &error));
    CommsTemplate back;
    REQUIRE(readCommsTemplate(file, &back, &error));

    // ---- example 1: every slot taken, none of them X or Y ----
    {
        Configuration dest;
        dest.bus[0].enabled = true;
        dest.setCommsPasswordSlot(1, A);
        dest.setCommsPasswordSlot(2, B);
        dest.setCommsPasswordSlot(3, deriveAccessKey(QStringLiteral("dest-pw-c")));
        dest.setCommsPasswordSlot(4, deriveAccessKey(QStringLiteral("dest-pw-d")));

        QString why;
        CHECK(!commsTemplatePasswordsFit(dest, back, &why));
        CHECK(!why.isEmpty());
        // And asking did not change anything - the four are exactly as set.
        CHECK(dest.commsPassword(1) == A);
        CHECK(dest.commsPassword(2) == B);
        CHECK(dest.commsPasswordSlotFor(X) == 0);
        CHECK(dest.commsPasswordSlotFor(Y) == 0);
    }

    // ---- example 2: slots 1 and 2 taken, 3 and 4 free ----
    {
        Configuration dest;
        dest.bus[0].enabled = true;
        dest.setCommsPasswordSlot(1, A);
        ...
        QString why;
        CHECK(commsTemplatePasswordsFit(dest, back, &why));
        adoptCommsTemplatePasswords(dest, back);
        // 1 and 2 untouched - a load NEVER overwrites a slot...
        CHECK(dest.commsPassword(1) == A);
        CHECK(dest.commsPassword(2) == B);
        // ...and the file's passwords landed in the free slots, so every
        // marked section the merge appends resolves to a real slot.
        CHECK(dest.commsPasswordSlotFor(X) == 3);
        CHECK(dest.commsPasswordSlotFor(Y) == 4);
        CHECK(dest.commsPasswordsInUse() == 4);
    }

    // ---- and the match case: a password already present costs no slot ----
    {
        Configuration dest;
        dest.bus[0].enabled = true;
        dest.setCommsPasswordSlot(1, A);
        dest.setCommsPasswordSlot(2, X); // one of the template's, already here
        dest.setCommsPasswordSlot(3, B);

        QString why;
        CHECK(commsTemplatePasswordsFit(dest, back, &why)); // only Y needs a slot
        adoptCommsTemplatePasswords(dest, back);
        CHECK(dest.commsPasswordSlotFor(X) == 2); // reused, not duplicated
        CHECK(dest.commsPasswordSlotFor(Y) == 4); // the one free slot
        CHECK(dest.commsPassword(1) == A);
        CHECK(dest.commsPassword(3) == B);
    }
}
...
int main(int argc, char **argv)
{
    ...
    testAMarkedMessageLoadsIntoADocument(dir);
    testALoadIntoADocumentHoldingConcealedMessages(dir);
    testATriggerConditionIsReportedRatherThanInvented(dir);
    testTemplatePasswordSlottingIsDynamic(dir);
    testTheFolderLayout();
    ...
    testTheSaveButtonWritesAConcealedMessage(dir);
