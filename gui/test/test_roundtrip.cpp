...
static void testProtectedDocument()
{
    ...
    // ---- the same document as a .ct3s: nothing legible left on disk ----
    {
        ...
        // KEYED and concealed, which is the case the refusal is for: a real
        // password exists, this session has not given it, and the writer must not
        // launder the message out under a new one. A section with no password at
        // all is a different case and saves — see test_protection's
        // testSavingAfterAGet.
        CHECK(concealed.anyKeyedSectionConcealed());
        CHECK(!concealed.saveSecureToFile(spath, opts, &err));
...
static void testConcealedSaveGuards()
{
    ...
    // ---- a plain Save As was a complete bypass of Protect Communication ----
    // saveSecureToFile has always refused a concealed document. saveToFile did
    // not, and it is the writer that emits indented JSON — so the way past not
    // knowing the password was File > Save As, pick .ct3, and read the
    // protected message's CAN ID and bit layout out of the result in Notepad.
    {
        Configuration concealed;
        ...
        err.clear();
        CHECK(!concealed.saveToFile(leak, &err));
        CHECK(!err.isEmpty());          // it has to say why, not just fail
        CHECK(!QFile::exists(leak));    // and it must not have written first

        // Unlocking the SECTION is the only thing that changes the answer. The
        // document-wide reveal is not: it opens no message at any tier, so a
        // session holding only that one still cannot write this file out.
        CHECK(concealed.revealProtectedComms(pass));
        CHECK(!concealed.saveToFile(leak, &err));
        CHECK(concealed.bus[0].sections.size() == 1);
        concealed.grantSectionAccess(0, concealed.bus[0].sections[0]);
        CHECK(concealed.saveToFile(leak, &err));
        CHECK(QFile::exists(leak));
    }
