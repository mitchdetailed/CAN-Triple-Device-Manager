...
static void testDeviceAccess(const SerialProtoCallbacks *restore)
{
    ...
    static_assert(sizeof(ct::AccessKeyRecord) == 13, "AccessKeyRecord must be 13 bytes");
    ...
    static_assert(sizeof(ct::AccessKeyWritePayload) == 6,
                  "AccessKeyWritePayload must be 6 bytes");
                    ...
                    writeKeyPayload(ct::ACCESS_FN_SEND, sendKey, false)));
    {
        ...
        if (packets.size() == 1) {
            // WHICH keys are set is answerable; what they ARE is not. One byte
            // comes back and there is no command that returns more.
            CHECK(packets[0].payload.size() == 1);
...
static void testConfigVersion(const SerialProtoCallbacks *restore)
{
    ...
    CHECK(engine_config_version() == 1337);
    {
        ...
        CHECK(flash_store_validate(nullptr, nullptr, nullptr, nullptr, &stored, nullptr, nullptr));
        CHECK(stored == 1337);
...
static void testMessageProtectionIsHostOnly(const SerialProtoCallbacks *restore)
{
    ...
    // ---- INVERSE 2: setting Edit Protected Comms changes nothing here ----
...
static void testAccessKeyDurability(const SerialProtoCallbacks *restore)
{
    ...
    CHECK(expectAck(ct::CMD_SAVE_TO_FLASH, QByteArray()));
    {
        ...
        CHECK(flash_store_validate(nullptr, nullptr, nullptr, &stored, nullptr, nullptr, nullptr));
...
static void testDeviceBinding(const SerialProtoCallbacks *restore)
{
    ...
    // ---- unbound (all-zero) runs anywhere: every pre-v18 config ---------
    {
        ...
        flashErase();
        CHECK(flash_store_commit(counts, nullptr, nullptr, nullptr, 0, nullptr, nullptr));
        CHECK(flash_store_validate(nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr));
        CHECK(flash_store_config_status() == ct::CONFIG_STATUS_OK);
    ...
    // ---- bound to A: runs on A ----------------------------------------
    {
        flash_store_set_device_uid(uidA);
        flashErase();
        CHECK(flash_store_commit(counts, nullptr, nullptr, nullptr, 0, uidA, nullptr));
        uint8_t readBack[ct::CONFIG_UID_LEN] = {0};
        CHECK(flash_store_validate(nullptr, nullptr, nullptr, nullptr, nullptr, readBack, nullptr));
        CHECK(flash_store_config_status() == ct::CONFIG_STATUS_OK);
    ...
    // ---- over the wire: GET_DEVICE_ID reports identity + why ------------
    {
        ...
        flash_store_set_device_uid(uidB);
        (void)flash_store_validate(nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
...
#undef T8_OFFSET_AGREES
    ...
    // firmware's declaration. ----
    {
        ...
        static_assert(sizeof(ct::CounterConfig) == 31, "counter record is 31 bytes");
        ct::CounterConfig g{};
...
#undef WRITE_CHUNK_IS_MAXIMAL
        ...
        CHECK(ct::WRITE_CHUNK_TIMERS == 24);
...
#undef MATH_OP_AGREES
    ...
    // v3 timer: accumulates while Engine RPM > 0.
    ct::TimerRow timer;
    timer.outputChannel = QStringLiteral("GP Raw 3");
    timer.startChannel = QStringLiteral("Engine RPM");
